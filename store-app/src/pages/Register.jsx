...
export default function Register() {
  const [form, setForm] = useState({ name: '', email: '', password: '', role: 'store', store_name: '' })
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        <div style={s.brand}>StorePanel</div>
        <h1 style={s.title}>Crear tienda</h1>
        <p style={s.sub}>Registra tu tienda en la plataforma</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input style={s.inputHighlight} type="text" placeholder="Nombre de tu tienda"
            value={form.store_name} onChange={e => setForm({...form, store_name: e.target.value})} required />
          <button style={s.btn} type="submit" disabled={loading}>{loading ? 'Creando...' : 'Crear tienda'}</button>
        </form>
...
const s = {
  ...
  brand: { fontSize: '13px', fontWeight: '600', color: '#7c3aed', letterSpacing: '0.05em', textTransform: 'uppercase', marginBottom: '1.5rem' },
  ...
  inputHighlight: { width: '100%', padding: '0.65rem 0.85rem', border: '2px solid #7c3aed', borderRadius: '8px', marginBottom: '1rem', outline: 'none', background: '#fafafa' },
  btn: { width: '100%', padding: '0.75rem', background: '#7c3aed', color: 'white', border: 'none', borderRadius: '8px', fontWeight: '600', marginTop: '0.25rem' },
